...
*Example: Claude Code System Prompt Leaked* (link is no longer active) - there are repos and collections of System Prompts from the major LLMs that come out very quickly after the new LLMs are released.  They are quite interesting and educational to see what base instructions they are given to work with, but also to see how these could potentially be manipulated.
